...
else
{
    ...
    //Use clockwise definitions!
    if (!position_meeting(x, y - 32, oStaticOccluder)) occluder.add_edge(_l, _t,   _r, _t); //Top
    if (!position_meeting(x + 32, y, oStaticOccluder)) occluder.add_edge(_r, _t,   _r, _b); //Right
    if (!position_meeting(x, y + 32, oStaticOccluder)) occluder.add_edge(_r, _b,   _l, _b); //Bottom
    if (!position_meeting(x - 32, y, oStaticOccluder)) occluder.add_edge(_l, _b,   _l, _t); //Left
}