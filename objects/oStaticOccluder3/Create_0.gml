...
else
{
    ...
    //Use clockwise definitions!
    if (!position_meeting(x, y - 32, oStaticOccluder3)) occluder.AddEdge(_l, _t,   _r, _t); //Top
    if (!position_meeting(x + 32, y, oStaticOccluder3)) occluder.AddEdge(_r, _t,   _r, _b); //Right
    if (!position_meeting(x, y + 32, oStaticOccluder3)) occluder.AddEdge(_r, _b,   _l, _b); //Bottom
    if (!position_meeting(x - 32, y, oStaticOccluder3)) occluder.AddEdge(_l, _b,   _l, _t); //Left
}