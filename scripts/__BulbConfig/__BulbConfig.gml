...
//Controls how shadow overlays (created with the BulbShadowOverlay() constructor) are drawn. The
//default value is <true>, where the HSV value (calculated as max(r,g,b)) is converted into an
//alpha value. This is analogous to a lighting sprite, only when using as a shadow overlay sprite,
//the effect is subtractive. If set to <false> then the alpha channel of the sprite is used.
//instead (and the RGB channels are ignored).
